...
class Aruco_Pose(Node): 
    ...
    f"callback_fps={self._fps:.1f}"
        ...
        if self.publish_debug_image:
            self._publish_debug(stamp, frame_small, detections, t0, detect_ms )
    ...
    def _open_camera(self, source, calib_size, threaded):
        ...
        if frame_size != calib_size:
            ...
            self.camera_matrix = calibration.fit_camera_matrix(self.camera_matrix,self.calib_size, frame_size)
        return frame_size
    ...
    def _publish_debug(self, stamp, frame, detections, t0, detect_ms):
        ...
        if detections:
            for det in detections:
                y = viz.draw_detection(frame, det, self.camera_matrix_small,
