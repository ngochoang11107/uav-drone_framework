...
class UsbCamera(CameraSource):
    ...
    def _open(self):
        ...
        if not cap.isOpened():
            # print("helloooooooooooooooooo1111111")
            return cap
...
class RtspCamera(CameraSource):
    ...
    def __init__(self, url, backend="ffmpeg", latency=100, protocol="tcp"):
        super().__init__()
        self.url = url
...
class ThreadedCamera:
    ...
    #                 fps = count / (now - last_time)

    #                 print(
    #                     f"[Camera thread] FPS = {fps:.2f}, "
...
def open_camera(source, width=None, height=None, threaded=True, **kwargs):
    """Build, wrap and start a camera source."""
    ...
    if threaded:
        return ThreadedCamera(src).start()
